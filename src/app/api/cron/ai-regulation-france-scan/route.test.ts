...
describe("cron france ai regulation scan route", () => {
  afterEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  ...
  it("rejects missing authorization header", async () => {
    ...
    const { GET } = await import("@/app/api/cron/ai-regulation-france-scan/route");
    const response = await GET(
      new Request("http://localhost/api/cron/ai-regulation-france-scan"),
    );
  ...
  it("defaults to the France official legal scan profile", async () => {
    process.env.CRON_SECRET = "1234567890abcdef";
    process.env.ADMIN_AUTH_SECRET = "123456789012345678901234";
  ...
  it("accepts the France live news profile explicitly", async () => {
    process.env.CRON_SECRET = "1234567890abcdef";
    process.env.ADMIN_AUTH_SECRET = "123456789012345678901234";
    process.env.APP_DATA_MODE = "supabase";
    process.env.NEXT_PUBLIC_SUPABASE_URL = "https://example.supabase.co";
    ...
    const response = await GET(
      new Request(
        "http://localhost/api/cron/ai-regulation-france-scan?profile=france_live_news_scan",
        {
          headers: {
            authorization: "Bearer 1234567890abcdef",
          },

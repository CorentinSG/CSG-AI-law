...
describe("cron ireland ai regulation scan route", () => {
  afterEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  ...
  it("rejects missing authorization header", async () => {
    ...
    const { GET } = await import("@/app/api/cron/ai-regulation-ireland-scan/route");
    const response = await GET(
      new Request("http://localhost/api/cron/ai-regulation-ireland-scan"),
    );
  ...
  it("defaults to the Ireland official legal scan profile", async () => {
    process.env.CRON_SECRET = "1234567890abcdef";
    process.env.ADMIN_AUTH_SECRET = "123456789012345678901234";
    ...
    expect(queueAndDrainScanJob).toHaveBeenCalledWith({
      ...
      trigger: "scheduled",
      requestedBy: "vercel-cron-ireland",
      scanProfile: "ireland_official_legal_scan",
      executionMode: "drain",
    });
    await expect(response.json()).resolves.toMatchObject({
      ok: true,

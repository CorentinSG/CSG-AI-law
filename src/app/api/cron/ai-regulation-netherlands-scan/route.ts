...
function getNetherlandsScanProfile(profileValue: string | null): NetherlandsAgentProfileId {
  if (profileValue && (netherlandsAgentProfileIds as readonly string[]).includes(profileValue)) {
    return profileValue as NetherlandsAgentProfileId;
  }
  return "netherlands_official_legal_scan";
}

async function handleScheduledNetherlandsScan(request: Request) {
  ...
  const scanProfile = getNetherlandsScanProfile(url.searchParams.get("profile"));
  const {
    queuedJob,
    processedJob,
    ...
    await queueAndDrainScanJob({
      sourceId,
      trigger: "scheduled",
      requestedBy: "vercel-cron-netherlands",
      scanProfile,
...
export async function GET(request: Request) {
  return handleScheduledNetherlandsScan(request);
}

...
describe("cron germany ai regulation scan route", () => {
  afterEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  ...
  it("rejects missing authorization header", async () => {
    ...
    const { GET } = await import("@/app/api/cron/ai-regulation-germany-scan/route");
    const response = await GET(
      new Request("http://localhost/api/cron/ai-regulation-germany-scan"),
    );
  ...
  it("defaults to the Germany official legal scan profile", async () => {
    process.env.CRON_SECRET = "1234567890abcdef";
    process.env.ADMIN_AUTH_SECRET = "123456789012345678901234";
    ...
    expect(queueAndDrainScanJob).toHaveBeenCalledWith({
      ...
      trigger: "scheduled",
      requestedBy: "vercel-cron-germany",
      scanProfile: "germany_official_legal_scan",
    });
    await expect(response.json()).resolves.toMatchObject({
      ok: true,
      regionScope: "Germany",

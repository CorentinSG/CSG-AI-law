...
function getSpainScanProfile(profileValue: string | null): SpainAgentProfileId {
  if (profileValue && (spainAgentProfileIds as readonly string[]).includes(profileValue)) {
    return profileValue as SpainAgentProfileId;
  }
  return "spain_official_legal_scan";
}

async function handleScheduledSpainScan(request: Request) {
  ...
  const scanProfile = getSpainScanProfile(url.searchParams.get("profile"));
  const {
    queuedJob,
    processedJob,
...
export async function GET(request: Request) {
  return handleScheduledSpainScan(request);
}

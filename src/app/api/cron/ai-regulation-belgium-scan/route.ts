...
function getBelgiumScanProfile(profileValue: string | null): BelgiumAgentProfileId {
  if (profileValue && (belgiumAgentProfileIds as readonly string[]).includes(profileValue)) {
    return profileValue as BelgiumAgentProfileId;
  }
  return "belgium_official_legal_scan";
}

async function handleScheduledBelgiumScan(request: Request) {
  ...
  const scanProfile = getBelgiumScanProfile(url.searchParams.get("profile"));
  const {
    queuedJob,
    processedJob,
...
export async function GET(request: Request) {
  return handleScheduledBelgiumScan(request);
}

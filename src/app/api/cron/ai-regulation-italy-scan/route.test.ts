...
describe("cron italy ai regulation scan route", () => {
  afterEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  ...
  it("rejects missing authorization header", async () => {
    ...
    const { GET } = await import("@/app/api/cron/ai-regulation-italy-scan/route");
    const response = await GET(
      new Request("http://localhost/api/cron/ai-regulation-italy-scan"),
    );
  ...
  it("defaults to the Italy official legal scan profile", async () => {
    process.env.CRON_SECRET = "1234567890abcdef";
    process.env.ADMIN_AUTH_SECRET = "123456789012345678901234";

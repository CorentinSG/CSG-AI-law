...
import { denmarkNationalDepthEntries } from "@/content/ai-regulation/denmark-national-depth";
import type { DenmarkNationalDepthEntry } from "@/content/ai-regulation/denmark-national-depth";
import type { RawRegulatoryItemInput, RegulatoryUpdateDraftInput } from "@/db/repository-types";
import { env } from "@/lib/env";

const SOURCE_ID = "denmark-national-depth";
const SOURCE_NAME = "Denmark national legal depth";
const SOURCE_URL = "https://csg-ai-law.vercel.app/ai-regulation/europe/denmark";
const DETECTED_AT = "2026-07-19T03:45:00.000Z";
const BACKFILL_TAG = "denmark-national-depth";
...
function rawIdentity(entry: DenmarkNationalDepthEntry) {
  return `${BACKFILL_TAG}:${slugify(entry.title)}:${stableHash(entry.sourceUrl).slice(0, 12)}:v1`;
}

function buildRawItem(entry: DenmarkNationalDepthEntry): RawRegulatoryItemInput {
  const rawText = [
    entry.summary,
    "",
    entry.whatHappened,
  ...
  return {
    ...
    rawText,
    rawMetadata: {
      baselineKind: "denmark_national_depth",
      country: "Denmark",
      ...
      traceability: {
        ...
        officialSource: true,
        parserUsed: "denmark_national_depth_backfill",
        scanTimestamp: DETECTED_AT,
        scanTrigger: "manual",
        scanProfile: "denmark_national_depth_backfill",
...
function buildUpdate(entry: DenmarkNationalDepthEntry, rawItemId: string): RegulatoryUpdateDraftInput {
  return {
    sourceId: SOURCE_ID,
    rawItemId,
    title: entry.title,
    ...
    affectedParties: [
      "Danish AI providers and deployers",
      "Danish public authorities",
      "Data protection officers",
      "Employers and workplace-monitoring operators",
      "Creative-sector rights holders",
      "Cloud and data infrastructure providers",
    ],
    keyObligations: [
      "Separate official national Danish sources from EU baseline entries and from media-only live news.",
      "Track follow-up Danish AI Act authority designations, Datatilsynet guidance, copyright AI measures, workplace-monitoring guidance and regulator updates.",
      "Map each item to the legal-area, authority-type and country axes so the Denmark page can be filtered by domain.",
    ],
    complianceDeadlines: [],
    enforcementRisk:
      entry.authorityType === "Binding law"
        ? "High relevance because this is a binding statute or legal-authority development for Denmark."
        : entry.authorityType === "Agency guidance"
          ? "Medium relevance because this is official Danish regulator or public-administration guidance likely to shape compliance practice."
          : "Medium relevance because this is official Danish policy or governance material that frames national implementation.",
...
async function ensureSource() {
  ...
  return updateRepository.addSource({
    ...
    lastScannedAt: null,
    notes:
      "Country-by-country official-source national depth for Denmark. Complements, but does not replace, live monitoring sources.",
...
async function main() {
  const dryRun = boolEnv(process.env.DENMARK_NATIONAL_DEPTH_DRY_RUN, true);
    ...
    `${JSON.stringify(
      {
        ...
        country: "Denmark",
        entryCount: denmarkNationalDepthEntries.length,
        results,
      },
      null,
      2,
...
main().catch((error) => {
  console.error(`[backfill-denmark-national-depth] failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});

...
>
> Ownership override (2026-07-25): Claude Code owns every unfinished item in
> this plan, including every backend, database, operations, frontend, and UX item. Railway actions are
> superseded by `docs/operations/github-actions-monitoring.md` and the total
> handoff document.

## Status legend

`DONE` (shipped on main this session) · `OPS` (needs an operator action in a console, no code) · `→CLAUDE` (Claude Code ownership) · Effort S/M/L/XL.
...
Historical Railway instructions below must not be executed as written. Railway
expired; configure and validate the GitHub Actions replacement described in the
2026-07-25 total handoff.
...
## Wave 1 — Database integrity (→CLAUDE, 1–2 weeks)
...
## Wave 2 — Monitoring reach (→CLAUDE, 2–3 weeks)

| # | Item | Effort |
|---|---|---|
| 2.1 | Rebase and split `handoff/claude-aggressive-news-radars-2026-07-25` (France, EU, New York press lanes) onto current `main`; validate seeds, selectors, source IDs, and publication guardrails | M |
...
## Wave 3 — Scale architecture (→CLAUDE, parallélisable)

...
| T-RT1A | Source staleness and failure detection backend | Codex 1 | claimed | `src/agents/ai-regulation/**` backend helper/test files, repository read surfaces if needed | `PROJECT_LOGBOOK.md`, `AI_AGENT_MASTER_CONTEXT.md`, `AGENT_COORDINATION.md` | low | 2026-06-10 ET | First task from the new Claude Code P-RT split. Boundary: backend-only per-source freshness/failure summaries from `source_health_checks`, `ingestion_logs`, scan logs, scheduler guidance. No admin UI, no `vercel.json`, no country editorial files. |

...
- Files: vitest.config.ts, eslint.config.mjs, tsconfig.json, scrapling_worker/{worker.py,README.md}, src/agents/ingestion/scraplingClient.ts, src/lib/env.ts, src/db/{repository-types,repositories/*}, src/agents/ai-regulation/{connectors/*,processors/pipeline.ts,scanProfiles.ts,scheduler/index.ts,citations.ts,+10 country agents}, src/content/ai-regulation/{news.ts,live-intelligence.ts}, src/app/{layout.tsx,[lang]/layout.tsx,feed.xml/route.ts,api/health,api/cron/ai-regulation-central-scheduler,4 dynamic page templates,[lang]/ai-regulation/page.tsx,admin/ai-regulation/page.tsx}, src/components/site/{hero-video-section,profile-portrait,lang-attribute}.tsx, src/lib/i18n/metadata.ts, playwright.config.ts, .github/workflows/ci.yml, package.json, README.md, DECISIONS.md (4 entries), docs/superpowers/plans/2026-07-20-master-improvement-plan.md.
- Graph anchors: `isLivePanelEligible()`, `buildApiScanIssueResult()`, `official_fast_scan` / `OFFICIAL_FAST_SOURCE_IDS`, `withSourceScanTimeout()`, `resetPisteTokenCacheForTests()`, `pageAlternates()`, `getCountryIntelligenceBySlug(slug, {scope})`, communities "Scan Pipeline", "Source Runtime Health", "Live Legal Intelligence Data", "API Connectors and Legal Docs".
- Verification: npm test 695/695 PASS (~13s) · lint PASS (0 problems) · typecheck PASS · production build PASS (with CI placeholder env; local .env has default admin creds by design) · e2e NOT run locally (config now CI-gated to `next start`; first CI run will exercise it).
- Branch/commit: `main` @ 10 local commits e20d19f..HEAD (push to origin pending — owner action or next session).
- Next: (1) OWNER ops actions listed in the plan doc Wave 0 (SCRAPLING_WORKER_TOKEN ×2, SCAN_JOB_ROUTE_ENQUEUE_ONLY=true, uptime monitor on /api/health?check=worker, NEWSAPI key check). (2) HANDOFF→Codex: Wave 1 DB integrity (migration 031 CHECK repairs is URGENT — national-depth ingestion violates the prod CHECK today; 016 damage assessment; migration runner; unified dedup; publication gate at creation; retention/backups; OpenAI real cost). Full backlog: docs/superpowers/plans/2026-07-20-master-improvement-plan.md. (3) Claude Code next: Wave 3.7 country-console generalization + Wave 4 i18n.

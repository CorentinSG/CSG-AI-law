...
- Files:         `src/content/ai-regulation/us-state-ai-law-baseline.ts`, `src/content/ai-regulation/us-legal-baseline.test.ts`
- Graph anchors: `baselineLegislatureSource()` (new), `prioritySources`, `createProfile()`, `usStateMapStatuses`
- Verification:  `npm test` 1010/1010 · lint clean · typecheck clean · build succeeds.
- Branch/commit: `claude/us-state-db-batch2` @ (this commit)
- Next:          Claude Code. All 51 jurisdictions now carry at least one runtime-verified official source (18 added, all HTTP 200), so no state sits at `needs_review` any more. Each new source note records whether a corpus sweep was run and the snapshot date behind it, so a reader can tell "swept a fresh corpus, found nothing" from "never swept" from "corpus too small or too old to sweep". The `does not present unverified states as confirmed enacted law` test lost its population and was restated as `gives every state an official source and never infers a law from one`. Still to verify at statute level: AK, GA, IN, IA, KS, ME, MA, MO, NV, NC, OH, OK, SC, VT, WV, WY, DC (17) plus AL. Indiana needs the bulk Code download.
...
- Verification:  `npm test` 1010/1010 · lint clean · typecheck clean · build succeeds.
- Branch/commit: `claude/us-state-db-batch2` @ (this commit)
- Next:          Claude Code, next batch. Added NH (RSA 638:26-a, 664:14-c, 507:8-j) and RI (Chapter 17-30) — 30 enacted states. NH is notable: its criminal deepfake offence is not confined to elections. Searched and found nothing: AL, IA. Indiana failed for the second time — iga.in.gov serves the Indiana Code only through client-side rendering, so neither WebFetch nor search returns statutory text; it needs a different access route (bulk Code download or an official PDF that the runner can parse). Two dead ends corrected mid-pass rather than trusted: RSA 664:14-a and 664:14-b are 2003/2009 telephone provisions, not the AI ones, and RI Public Law 2024 ch. 453 is the Data Transparency and Privacy Protection Act, not synthetic media. Remaining unpopulated: 20 states + DC.
...
- Verification:  `npm test` 1010/1010 · lint clean · typecheck clean · build succeeds.
- Branch/commit: `claude/us-state-db-batch2` @ (this commit)
- Next:          Claude Code, next batch. Added MN, PA, LA, ID — 26 enacted states now. LA is the first non-election deepfake statute in the set (sexual conduct and minors). PA and MN are drafted around digital forgery / deep fake technology and do not use the term "artificial intelligence"; that is recorded in their entries rather than glossed over. Legal Data Hunter is still rate-limiting every request, so all four were read on the issuing site. Remaining unpopulated: 24 states + DC.

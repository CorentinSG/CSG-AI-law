...
2026-06-12 - User decision for Claude Code / T-OPS3: put the permanent worker deployment on hold until the end of site construction. Interim direction: local worker on the user's computer may be used later for testing, but do NOT enable `SCAN_JOB_ROUTE_ENQUEUE_ONLY=true` in production yet; Railway/Fly remains the likely final production hosting choice.

2026-06-12 - User product direction noted: future site scope should expand beyond AI regulation to include legal intelligence for personal data/privacy law and cloud law, while preserving the current official-source vs discovery-only vs human-reviewed publication separation. Also recorded in `README.md`.

2026-06-12 - Codex, T-OPS4 (DONE): stabilized full-suite auth-route cold-import margin with Vitest `testTimeout: 10000` after no local repro of the documented 5s flakes; kept parallelism intact. Added deterministic golden fixtures under `src/agents/ai-regulation/connectors/fixtures/` for Legifrance PISTE search JSON, EUR-Lex AI Act HTML, and official RSS XML for Garante/BfDI/AgID/IMY; tests now read fixtures from disk and verify PISTE URL mapping, EUR-Lex CELEX/article/annex extraction, and RSS AI-term filtering with the real parser. Temporary CID mutation in the PISTE fixture made `api-connector.test.ts` fail on the expected URL, then was reverted and re-verified. Verification: targeted connector/route tests pass; 3 consecutive full `npm test` runs passed (469 tests), final post-mutation `npm test` passed, plus `npm run lint`, `npm run typecheck`, and `npm run build` pass.

2026-06-12 - Codex, T-OPS6 (DONE): added production `GET /api/health` (`src/app/api/health/route.ts`) backed by `src/lib/health.ts`. Public unauthenticated response returns coarse booleans/ages only; `Authorization: Bearer $CRON_SECRET` returns private details including per-profile-family successful scan ages and pending `needs_review` count. Health uses cheap repository reads (`scanLogs`, `scanJobs`, one-page `needs_review` count), reports worker heartbeat from persisted scan-job lease heartbeat when available, app version from `package.json`, and commit from deployment env (`VERCEL_GIT_COMMIT_SHA`/fallbacks). DB errors return 503 with details only on the private tier. Verification: `npm test` (465), `npm run lint`, `npm run typecheck`, `npm run build` all pass.

2026-06-12 - Codex, T-OPS2 (DONE): delivered outbound webhook alerting via new `src/lib/alerting.ts`. `ALERT_WEBHOOK_URL` is optional/off when unset and documented in `.env.example`; payloads carry source ids/names, state, counts, statuses, timestamps only. Hooks: source finalization alerts on stale/degraded transitions and >=3 consecutive source failures; scan-job stewardship emits a once-per-day review-backlog digest guarded by an `ops-alerting` scan-log marker (no new table). Webhook delivery/evaluation failures are swallowed so scans keep succeeding. Verification: `npm test` (459), `npm run lint`, `npm run typecheck`, `npm run build` all pass.
...
T-OPS4 complete. T-OPS6 complete. T-OPS2 complete. T-OPS1 complete (branch pushed; PR awaiting user open+merge). Codex P-OPS sequence complete. Claude Code: T-OPS3 intentionally on hold until end of site construction; do not enable production enqueue-only yet. T-OPS5/T-OPS7 already have completion notes above.

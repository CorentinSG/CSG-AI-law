...
| T-DURABLE-DATA | Codex | DONE-LOCAL | `ops/t-ops9-ux` @ `bd3a139` | durable-data backend commits; release-gate documentation | `evaluateSchemaIntegrity()`, `AiRegulationRepository`, `executeClaimedScanJob()`, community "DB Repository Layer", community "Scan Job Management", community "Data Ingestion Pipeline" | 2026-07-02 |
...
- Branch/commit: `ops/t-ops9-ux` @ `bd3a139`; durable-data implementation commits `4d7f2d2..bd3a139`; release handoff commit `docs(ops): hand off durable data phase`.
- Next:          Operator must provide a disposable database, apply migrations 001-014 twice there, rerun the schema audit, exercise concurrent raw insertion and stale-token completion, and verify a logical restore. Applying production migrations 013-014 and running a post-migration official-source scan require separate explicit approval. Claude owns the three UI lint errors.

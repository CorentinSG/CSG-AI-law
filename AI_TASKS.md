...
| T-CONTACT-REDESIGN | Claude Code | REVIEW | `claude/contact-page-redesign-u2sh6q` @ `f1f884a` | `src/app/[lang]/contact/page.tsx`, `src/components/site/contact-experience.tsx` | `ContactPage`, `ContactExperience` (new), `SpotlightHover`, community "UI Components and Utilities" | 2026-07-23 |
...
- Branch/commit: `claude/github-monitoring-recovery-lz4dos` @ (see git log)
- Next:          Remaining: HR, CZ, EE, GR, PT, RO, SK (+ HU partial). NEGATIVE FINDINGS 2026-08-08 (do not redo): LDH quota had reset and was usable, but it is the WRONG TOOL for these seven — its GR corpus is the legacy Raptarchis/huggingface set (gazette coverage stops at 2024), so law 5321/2026 is absent; and a PT/DiarioRepublica sweep of Aug-Nov 2025 surfaced no AI Act designation instrument, consistent with reports that Portugal designated ANACOM without a published RCM. RO/CZ/SK/HR/EE all hinge on memoranda or unadopted bills, which enacted-law corpora do not carry by construction. These seven need GOVERNMENT WEB PAGES (anacom.pt, ancom.ro, ai.gov.gr, mirri.gov.sk, chd.lu-style dossiers), which only Exa's server-side fetch can reach — container egress denies every gazette host tested (et.gr, ai.gov.gr, anacom.pt, ancom.ro, mirri.gov.sk, slov-lex.sk, narodne-novine.nn.hr, riigiteataja.ee). So: gate the next attempt on Exa being connected, not on LDH quota. CHANNEL INVENTORY CLOSED 2026-08-08: (1) LDH — quota fine, corpus wrong for these seven; (2) container egress — CONNECT 403 on all eight gazette hosts; (3) Playwright MCP — looks for Chrome at /opt/google/chrome/chrome which this image lacks (it ships /opt/pw-browsers), and would use the same proxy anyway; (4) WebFetch — confirmed to route through the same proxy, returns an explicit {"error_type":"EGRESS_BLOCKED"} for ai.gov.gr. Only WebSearch (Anthropic-side, snippets only — not citable as an official source) and Exa's server-side fetch escape the proxy. Exa is therefore the sole viable channel and it flaps. PASSIVE WAITING CLOSED 2026-08-09: five scheduled probes (08-08 09:00, 15:02, 21:05; 08-09 03:09, 09:11 UTC) all found Exa disconnected. No further probes armed. Unblocking now requires an operator action — reconnect Exa in claude.ai connector settings, or allowlist the eight gazette hosts in the environment's egress policy. Do not write these seven profiles from WebSearch snippets; they are not citable official sources.
...
- Next:          Claude Code continues country coverage. Corrected count: 12 profiles carry zero national measures (HR, CZ, EE, GR, LV, LT, LU, MT, PT, RO, SK, SI); CY and HU are partial. Blocked 2026-08-07: Legal Data Hunter free daily quota exhausted, and this session's egress policy denies CONNECT to eur-lex.europa.eu, digital-strategy.ec.europa.eu and every national gazette host tested, so no primary source is reachable to verify designations. Correction (2026-08-07): the 8 `*-national-depth.ts` files are NOT dead code — each is consumed by its `scripts/backfill-<country>-national-depth.ts` seed script (7 sourced entries per country, seeded 2026-07-19). The earlier dead-code claim came from grepping only `src/`. Do not edit those content files casually: entries are hash-keyed seeds already pushed to the DB. Owner: watch the Supabase egress slope before 20 August.
...
2026-08-05 · Claude Code · T-A50-HYDRATION · RESOLVED
- Intent: Root-caused and fixed the hydration failure reported below. Cause is `src/app/[lang]/ai-regulation/loading.tsx`. On Next 16.2.6 a `loading.tsx` creates a Suspense boundary whose subtree never finishes hydrating in a production build: React keeps the fallback shell, the streamed server HTML stays in the DOM with no fiber owner, and every control beneath it is dead. No console error, no failed request — the page looks correct and simply does not respond.
- Not route-specific: proved general by adding a trivial `loading.tsx` to the healthy `/eu-ai-act` route, which broke that route too. The earlier relocation of the Article 50 checklist therefore worked by accident, not by design — that route happened to have no loading file.
- Also ruled out along the way (all tested on clean production builds): the page code itself, shell components, metadata, own and sibling `revalidate`, `generateStaticParams`, dynamic siblings, segment depth, middleware and response headers, Turbopack cache, service workers, and `SiteShell` duplication inside the fallback (removing SiteShell from loading.tsx did NOT fix it — only removing the file did).
- Files: deleted `src/app/[lang]/ai-regulation/loading.tsx`; added `src/app/no-loading-boundaries.test.ts`, which fails if any loading file reappears anywhere under src/app.
- Graph anchors: `HubScrollNav`, `CountryConsole`, community "AI Regulation Hub"
- Verification: test 1008/1008, lint/typecheck/build clean. On a production build, `/ai-regulation/europe/france` went from h1 fibers 0 → 2, orphaned DOM 1 → 0, and its filter pills now toggle (`aria-pressed` false → true); the scroll-spy marks a section active, which means its IntersectionObserver effect runs at all — it did not before.
- Next: the cost is the loading skeleton on hub navigation. Worth retesting on Next 16.3.0 (we are on 16.2.6): if the boundary hydrates there, restore `loading.tsx` and delete the guard test. Do not restore it without verifying interactivity on a real production build — dev mode hides this entirely.
...
- Recovery:      The probe now searches the site root and up to 8 listing links in the authority's own language when the seeded URL yields nothing (run 30397833023 found 7). Wired with a verified selector: `src-no-dpa-ai`, `src-at-dsb-ai`. Repointed at a verified live page but kept gated because the evidence showed reachability, not precision: `src-ch-dpa-ai` (126 items at 77% dated behind "Mehr über" wrappers), `src-fi-government-ai` and `src-se-digg-ai` (general agency news). Rejected: `src-ee-dpa-ai` (the listing is a brittle filtered search query) and `src-va-dpa-ai` (the newsletter lists Vatican coins and the Observatory — dated, well-formed and entirely unrelated). Gated totals 65 → 63.
- Verification:  `npm test` **849/849 across 138 files**, `npm run lint` clean, `npm run typecheck` clean, `npm run build` PASS (with CI's placeholder env).
- Branch/commit: `claude/github-monitoring-recovery-lz4dos` @ `a8b90c9` (PR #45)
- Next:          Claude Code. The remaining 16 unloadable lanes are a **connector** problem, not a seed one: 7 are 403s, 5 are TLS chain failures (`UNABLE_TO_VERIFY_LEAF_SIGNATURE` — Node does not fetch missing intermediates the way browsers do), 1 is a connect timeout, and `src-ie-dete-ai` serves an empty body because it is client-side rendered. `scanStaticSourceWithBrowserFallback()` already covers these on paper — a 403 throws in `StaticPageConnector` and every generated shell satisfies `shouldUseStaticBrowserFallback()` — but the fallback yields a whole-page `static_fallback_document` snapshot, not a listing, so it restores reachability without restoring extraction. Whether it fires at all in production is unverified and needs the read-only `DATABASE_URL`.
...
- Files:         `src/app/[lang]/{legal-notice,privacy,about}/page.tsx` (new), `src/lib/i18n/live-panel-copy.ts` (new), `src/components/site/{skip-to-content.tsx}` (new), `src/app/opengraph-image.tsx` (new), `src/db/repositories/supabase-repository-fallbacks.test.ts` (new), `src/agents/ai-regulation/euMemberStateSourceMandates.test.ts` (new), plus edits to `scheduler/index.ts`, `supabase-repository.ts`, `citations.ts`, `verification.ts`, `alerting.ts`, `sitemap.ts`, `section-heading.tsx`, `shell.tsx`, `news-card.tsx`, `country-console.tsx`, `live-legal-intelligence-panel.tsx`, `app/layout.tsx`, the country/state templates and the central-scheduler cron route.
- Graph anchors: `reportRepositoryFallback()` (new), `parseSourceReferences()`, `extractVerificationMetadata()`, `buildCentralMonitoringSchedule()`, `livePanelCopy` (new), `SkipToContent` (new). Graph rebuild pending — `graphify-out/` is absent from this clone and `graphify` is not on PATH, so no graph query informed this work.
- 2.2/2.11:      The scheduler enqueued 9 of 37 declared profiles. The 9 country official profiles already ran via `vercel.json` crons, so the dead set was 18 country live/verification profiles — 36 sources across Germany, the Netherlands, Belgium, Austria, Sweden and Ireland were in no executed profile. Added a data-driven `country` region, one country activated per cycle on a deterministic rotation (+2 jobs/cycle, not an 18-item burst). `findRecentDuplicateSchedulerJob` read 100 `scan_jobs` rows *inside* the plan loop — 900 rows/cycle, 2700 once country profiles landed; now one 300-row read. That is the load profile behind the 2026-07-27 Cloudflare 522. `euMemberStateSourceMandates` entries carry a required `provisioning` field so declared-but-unwired cannot read as coverage.
- 4.3/4.9:       New bilingual `/legal-notice` (LCEN + Attorney Advertising), `/privacy` (GDPR) and `/about`. Every owner-specific fact is a `{{OWNER: …}}` placeholder — 43 in total; nothing about the attorney is invented. All three are `noindex`, carry a draft banner and are absent from navigation until answered. The privacy notice describes the site as it is: no contact form exists, so it covers email correspondence, host logs and the single admin cookie. Franglais copy moved to a locale-keyed module, and `lang` is now passed at all 9 panel call sites — without that the fix was inert on `/fr`.
- 4.4/4.6/4.8:   77 pages gained an `h1` (not 78 — France already had one via `CountryConsoleHero`). `sitemap.ts` is now async and includes the DB-backed `/news/[slug]` and `/ai-regulation/[id]` routes, degrading to static routes if the repository throws. Skip-to-content link added in `SiteShell`. Open Graph moved to file-based `app/opengraph-image.tsx`: root `openGraph.images` would have been silently dropped because Next 16 merges metadata shallowly and every country/state/news/research page defines its own `openGraph`.
- 1.7/1.14:      `sourceReferences` and `VerificationMetadata` are Zod-validated at read time with conservative defaults, fixing real crashes (`isInaccessible()` calling `.includes()` and `publicationEligibility.ts:131` calling `.replaceAll()` on partial blobs). 53 silent legacy fallbacks in `supabase-repository.ts` are now loud and classified: **17 `write_not_persisted`** (the write never reached Postgres — `review_events`, `verification_attempts`, `scan_jobs`, `news_items`, `data_quality_findings`, `source_health_checks` and more), 7 `write_degraded` (persisted but lost columns or atomicity), 29 `read_degraded`. No fallback deleted — turning schema drift into a production crash would be worse. A static-analysis test asserts every detector branch is followed by a report call, with a `>= 49` floor so it cannot pass vacuously.
- Verification:  On a settled tree after all four agents finished: `npm test` **777/777 across 134 files** (was 743/131, +34), `npm run lint` clean, `npx tsc --noEmit` clean, `npm run build` PASS. Intermediate figures the agents reported (694/749/771) were distorted by concurrent writes and should be ignored.
- OPEN RISK:     The conservative read-time defaults are a real behaviour change for already-partial rows. `news.ts:250` does `verification?.officialSourceFound ?? <derive from citations>`; a partial blob previously left this `undefined` and fell through, now it is `false` and does not. Malformed rows become more likely to be withheld, never more likely to be published — consistent with the publication policy — but **some currently-public news items could become non-public** if production rows carry partial verification blobs. Unmeasurable without database access. Owner informed; decision pending.
- Corrections:   Two of my own briefs were wrong and were corrected with evidence. (a) The contrast premise: `text-white/50` is 5.33:1 and passes AA; the failure boundary is below `white/45`. The real issue is the zinc remapping in `globals.css` (`text-zinc-300` = 4.05:1, 31 public uses) and a blanket floor there would invert the palette hierarchy — left as a design decision. (b) Root `openGraph.images` would not have worked, see 4.8 above.
- Branch/commit: `claude/github-monitoring-recovery-lz4dos` @ `d5e569e` (commits `212f194`, `0601c37`, `1b38613`, `d5e569e`). Not merged.
- Next:          OWNER — answer the 43 `{{OWNER: …}}` placeholders, then the legal pages can be indexed and linked; decide on the publication-visibility risk above; decide on the zinc contrast remap; supply a read-only session-pooler `DATABASE_URL` so Wave 1 production forensics can start. Claude Code — 13 further public pages still lack an `h1` (one-line `as="h1"` each now the prop exists), `/news` sits in the sitemap but is a pure redirect, `usJurisdictionSourceMandates.ts` has the same unmarked aspirational shape as the EU file, and `conditional-fetch.ts` still has no per-request fetch timeout.
...
- Files:         `src/agents/ai-regulation/euNewsSources.ts` (1 line), `src/agents/ai-regulation/connectors/conditional-fetch.ts` (+74), `src/agents/ai-regulation/registry-seed-integrity.test.ts` (new), `src/agents/ai-regulation/connectors/conditional-fetch.test.ts`, `src/agents/ai-regulation/sourceRuntimeHealth.test.ts`.
- Graph anchors: `fetchTextWithConditionalCaching()`, `isTransientFetchStatus()`, `isTransientFetchError()`, `getTransientRetryDelayMs()`, `buildSourceExecutionDecisions()`, `getSourceDescriptor()`, `getFallbackCadence()`, `euNewsSourceRegistry`, community "Source Runtime Health". NOTE: the Graphify graph could not be consulted — `graphify-out/` is gitignored and absent from this clone and `graphify` is not on PATH, so exploration was by direct search. Rebuild the graph before relying on it; these anchors are new/changed nodes.
- 2.8 (fixed):   `euNewsSources.ts` declared `src-council-of-europe-ai` while the seed defines `src-council-europe-ai`. Registry `sourceId` is joined to `regulation_sources.id` in `getSourceDescriptor()` and in `euProfileSourceIds`, so the Council of Europe source was silently absent from `eu_official_legal_scan` and fell back to a generic cadence descriptor. Audited all 33 registries: this was the only orphan of 179 entries carrying a sourceId. Regression test added (verified RED before the fix, GREEN after).
- 2.3 (verified, no change needed): the plan's premise is stale. `scanFrequency` IS honoured at selection time — `buildSourceExecutionDecisions()` gates on `parseCadenceToMs(recommendedCadence)` and `scanSourcesForCandidates` skips when `shouldScan` is false. Measured the two paths: of 59 registry-covered seeded sources the registry cadence is faster in 45 and identical in 14 — **never slower**, so no source is under-scanned by the override; the other 179 sources use `getFallbackCadence()`, which maps all four declared frequencies correctly. Federal Register now runs at its declared `every_6_hours`. Added 6 table-driven tests pinning the fallback gating per frequency (previously only a registry-covered case was covered).
- 2.4 (implemented): no retry existed anywhere in the connectors. Added bounded retry-with-full-jitter inside `fetchTextWithConditionalCaching()` — the shared choke point for RSS, static-page and API/JSON connectors — for 502/503/504 and transient socket/DNS codes (nested `cause` inspected). 3 attempts, 500ms base, 4s cap. Non-transient statuses (incl. 404/429/500) and non-transient errors are never retried. A blip that recovers is an ordinary success, so it never increments `consecutiveFailures` and never triggers the source-level exponential backoff or circuit breaker — this is the "don't double backoff" requirement.
- Verification:  `npm test` PASS 741/741 (132 files, was 723/131 — 18 added), `npm run lint` PASS, `npm run typecheck` PASS, `npm run build` PASS (needs local env stubs: `APP_DATA_MODE`, `ALLOW_MEMORY_MODE_IN_PRODUCTION`, `ADMIN_AUTH_SECRET`, non-default `ADMIN_USERNAME`/`ADMIN_PASSWORD` — throwaway values, nothing written to disk). The 2.8 regression test was confirmed RED against the pre-fix id before being taken GREEN.
- Findings (not fixed, deliberately): (a) the US monitoring agent definitions declare ~217 source ids of which only the seeded New York lane resolves — the rest are the unseeded state lanes of plan item 2.6, so the strict orphan assertion deliberately excludes them; (b) `src-us-courtlistener-ai`, `src-us-nyc-oti-ai` and `src-us-ny-raise-act` are seeded and active but claimed by no US agent definition (still scanned by the generic profile pipeline at their declared daily cadence — a lane-grouping gap for 2.6, not an outage), pinned by a tripwire test so the list cannot grow unnoticed; (c) the ai-regulation connectors have no per-request fetch timeout — `conditional-fetch.ts` passes no `AbortSignal`, so Wave 0.7's `withSourceScanTimeout` is not present in the current tree. Retries do not worsen this (a hang blocks the first attempt regardless) but a hung source can still stall a scan.
- Branch/commit: `claude/github-monitoring-recovery-lz4dos`.
- Next:          Claude Code. Not merged. T-GITHUB-MONITORING-RECOVERY remains the priority and is still blocked on the operator adding the three Supabase secrets.
...
- Files:         `src/agents/ai-regulation/countryLiveDataRegistry.ts` (new), `src/agents/ai-regulation/euLegalNewsAgent.ts`, `src/app/[lang]/ai-regulation/europe/[country]/page.tsx` (generic live block after `#intel`), `europe/page.tsx`, `united-states/page.tsx`, `united-states/[state]/page.tsx`, `international/page.tsx`.
- Graph anchors: `getCountryLiveIntelligenceLoader()`, `getEuropeLiveLegalIntelligenceData()`, `buildLiveStoryFeed()`, `LiveLegalIntelligencePanel`, `EuropeCountryPage`, community "Intelligence Hub UI". (Graph rebuild pending; hook absent in this remote container.)
- Verification:  `npm test` PASS (709 / 131 files), `npm run lint` PASS, `npm run typecheck` PASS, `npm run build` PASS (memory verification env).
- Branch/commit: `claude/worldmonitor-csg-architecture-cj0by1` @ `1929be0`.
- Next:          Branch ready for review/merge as one worldmonitor-blueprint unit (5f00f9d + 054d52c + 1929be0). Non-EU European country pages (Western Balkans etc.) have no agents yet — extend the registry when those agents exist.

2026-07-23 - Claude Code - T-CROSS-SOURCE-CORROBORATION - DONE-LOCAL
- Intent:        Backend half of the worldmonitor blueprint, taken over from the earlier HANDOFF→Codex because Codex is out for a few days (user instruction). Ingestion-time cross-source corroboration: each new update is fuzzy-matched (story similarity, same geography, 14-day window) against a 400-row recent-update window plus same-batch siblings; matches from serious sources (official/regulator/reputable press — never informal discovery, never rejected/archived items) add reciprocal "supporting" references on both raw items, so `buildNewsItemFromUpdate` derives `corroborated` status and the standing multi-source auto-publish policy applies immediately. AI planning gives corroborated candidates a capped rank boost (reorders the AI_MAX_ITEMS_PER_SCAN slots only — no guardrail weakened). Worker live-cadence hard floor lowered from a fixed 15 min to a 5-min safety floor honouring SCAN_JOB_WORKER_SCHEDULER_INTERVAL_MS (default unchanged at 15 min; per-source cadence throttle in `buildSourceExecutionDecisions` still governs each source).
- Files:         `src/agents/ai-regulation/processors/crossSourceCorroboration.ts` (+ test, 8), `processors/pipeline.ts` (corroboration stage in `processAllCandidates` + reciprocal helper), `processors/aiPlanning.ts` (+ test), `processors/pipeline.harness.test.ts` (mock additions), `src/agents/ai-regulation/storyClustering.ts` (exported `isSameStoryTitle`), `scripts/run-scan-job-worker.ts`.
- Graph anchors: `findCorroboratingUpdates()`, `buildCorroborationMetadataPatch()`, `applyReciprocalCorroboration()`, `rankCandidateForAi()`, `isSameStoryTitle()`, community "Scan Pipeline", community "Data Ingestion Pipeline". (New nodes — graph rebuild pending; hook absent in this remote container.)
- Verification:  `npm test` PASS (709 / 131 files), `npm run lint` PASS, `npm run typecheck` PASS, `npm run build` PASS (memory verification env).
- Branch/commit: `claude/worldmonitor-csg-architecture-cj0by1` @ `054d52c`.
- Next:          Operator may set `SCAN_JOB_WORKER_SCHEDULER_INTERVAL_MS=300000` on Railway for a 5-min live loop when desired. Remaining backlog for whoever picks it up: persisted story identity across scans (mention counts / first-seen), per-source high-water-mark for >maxItems bursts. Codex on return: review the corroboration stage, no schema changes were needed.
...
- Files:         `scripts/backfill-eu-country-domain-baseline.ts`, `src/db/migrations/030_add_cloud_infrastructure_legal_area.sql`, `src/db/schema.ts`, `src/agents/ai-regulation/utils/classification.ts`, `src/agents/ai-regulation/utils/classification.test.ts`, `src/agents/ai-regulation/processors/openaiProcessor.ts`, `src/agents/ai-regulation/connectors/api-connector.ts`, `src/agents/ai-regulation/connectors/static-page-connector.ts`, `package.json`, `PROJECT_LOGBOOK.md`, `AI_AGENT_MASTER_CONTEXT.md`, `AI_TASKS.md`.
- Graph anchors: `legalAreas`, `inferLegalArea()`, `classifyEurLexLegalArea()`, `updateRepository.createRawItem()`, `updateRepository.createUpdate()`, `SourceReference`, community "DB Repository Layer", community "Scan Pipeline".
- Verification:  dry-run PASS; live Supabase run created/published 162 entries tagged `eu-country-domain-baseline`; idempotence replay PASS (`skipped_existing_update` x162); post-run EU matrix PASS for all 27 Member States and all 6 domains; 73 existing cloud/compute/infrastructure signals reclassified to `Cloud and infrastructure`; `npm run typecheck` PASS; `npm run lint` PASS; targeted tests PASS (11); `npm test` PASS (678); preview-env `npm run build` PASS.
- Branch/commit: `main` @ working tree.
- Next:          Codex should continue national depth country-by-country: national AI Act implementation measures, competent/market-surveillance/notifying authorities, DPA decisions/sanctions, national court decisions, copyright/generative-AI litigation, workplace/platform-work AI instruments, and reliable live legal news. Claude can safely consume the new `legalArea` in UI filters; do not label this baseline as national completeness.
...
2026-07-18 · Claude Code · T-LEGALDB-EXPLORER-UX · MERGED
...
- Files:         `src/agents/ai-regulation/internationalNewsSources.ts`, `src/agents/ai-regulation/internationalLegalNewsAgent.ts`, `src/agents/ai-regulation/scanProfiles.ts`, `src/agents/ai-regulation/scheduler/index.ts`, `src/agents/ai-regulation/globalMonitoringSupervisorAgent.ts`, `src/agents/ai-regulation/regionalManagerNeeds.ts`, `src/db/schema.ts`, `src/db/seed/ai-regulation-seed.ts`, `src/db/migrations/026_international_monitoring_sources.sql`, tests/docs.
- Graph anchors: `internationalMonitoringSourceRegistry`, `buildCentralMonitoringSchedule()`, `selectSourcesForScanProfile()`, community "Scan Pipeline", community "DB Repository Layer".
- Verification:  `npm run typecheck` PASS; `npm run lint` PASS; `npm test -- --maxWorkers=1` PASS (669 tests / 118 files); `npm run build` PASS with production memory-mode verification env.
- Branch/commit: `main` @ committed.
- Next:          Migration 026 equivalent applied to Supabase prod via service-role upsert (6 active International sources). After deploy, enqueue `international_official_legal_scan` and `international_live_news_scan` to gather source-health proof and populate the International hub.

2026-07-16 - Codex - T-JUDILIBRE-OAUTH-FALLBACK - DONE-LOCAL
- Intent:        Make the France case-law connector match the credentials now configured by Claude/operator: Judilibre still prefers direct `JUDILIBRE_API_KEYID`, but can also use the existing PISTE OAuth client credentials already configured for Legifrance.
- Files:         `src/agents/ai-regulation/connectors/api-connector.ts`, `src/agents/ai-regulation/connectors/api-connector.test.ts`, `src/agents/ai-regulation/agentApiCapabilities.ts`, `src/agents/ai-regulation/agentApiCapabilities.test.ts`, `scripts/verify-ingestion-runtime.ts`, `.env.example`, `src/db/seed/ai-regulation-seed.ts`, `src/agents/ai-regulation/regionalManagerNeeds.ts`, `README.md`, `PROJECT_LOGBOOK.md`, `AI_AGENT_MASTER_CONTEXT.md`, `AI_TASKS.md`.
- Graph anchors: `ApiConnector`, `listAgentApiCapabilities()`, community "API Connectors and Legal Docs", community "Agent API Capabilities".
- Verification:  `npm test -- src/agents/ai-regulation/connectors/api-connector.test.ts src/agents/ai-regulation/agentApiCapabilities.test.ts` PASS (2 files / 22 tests); `npm run typecheck` PASS; `npm run lint` PASS; `npm test -- --maxWorkers=1` PASS (117 files / 663 tests); preview-env `CIRCLE_NODE_TOTAL=2 npm run build` PASS.
- Branch/commit: `main` @ working tree.
- Next:          Claude/operator do not need to put PISTE/Judilibre credentials on `scrapling-worker`; rotate the exposed PISTE secret after live verification because it appeared in chat text.

2026-07-16 - Codex - T-PISTE-SANDBOX-FALLBACK - DONE-LOCAL
- Intent:        Turn the live PISTE validation failure into a durable fix: the configured PISTE app is sandbox, so PISTE-backed connectors must retry sandbox OAuth and call `sandbox-api.piste.gouv.fr` when production OAuth returns 400; Legifrance source is also activated as a first-class API source in seed and migration 025.
- Files:         `src/agents/ai-regulation/connectors/api-connector.ts`, `scripts/verify-ingestion-runtime.ts`, `src/db/migrations/025_activate_legifrance_piste_source.sql`, `src/db/seed/ai-regulation-seed.ts`, `PROJECT_LOGBOOK.md`, `AI_TASKS.md`.
- Graph anchors: `ApiConnector`, community "API Connectors and Legal Docs", community "DB Repository Layer".
- Verification:  Live Supabase jobs `job-bfda00e1-d356-41ca-8561-9f5715b54925` (Judilibre) and `job-7b5ab5fa-37ea-4703-9a55-3da2c698505c` (Legifrance) were drained by the worker but showed `PISTE OAuth token request failed with 400`; `src-fr-legifrance-ai` was updated in prod to `preferred_extraction_method=api`; `npm test -- src/agents/ai-regulation/connectors/api-connector.test.ts src/agents/ai-regulation/agentApiCapabilities.test.ts` PASS (2 files / 22 tests); `npm run typecheck` PASS; `npm run lint` PASS.
- Branch/commit: `main` @ working tree.
- Next:          Push/deploy this fallback, then re-enqueue `src-fr-judilibre-ai` and `src-fr-legifrance-ai` to confirm sandbox OAuth live and inspect source health/items.

2026-07-16 - Codex - T-INTERNATIONAL-HUB - DONE-LOCAL
- Intent:        Add International as a first-class AI Regulation hub alongside Europe and United States, with overview portal card, database-region card, sitemap entry, and route smoke coverage.
- Files:         `src/app/[lang]/ai-regulation/international/page.tsx`, `src/app/[lang]/ai-regulation/page.tsx`, `src/components/site/region-portal-card.tsx`, `src/app/sitemap.ts`, `e2e/smoke.spec.ts`, `README.md`, `PROJECT_LOGBOOK.md`, `AI_AGENT_MASTER_CONTEXT.md`, `AI_TASKS.md`.
- Graph anchors: `InternationalAiRegulationPage`, `RegionPortalCard`, `internationalAiStandardsBaseline`, community "Intelligence Hub UI", community "News and Regulation Admin".
- Verification:  `npm run typecheck` PASS; `npm test -- src/content/ai-regulation/international-ai-standards.test.ts` PASS (3 tests); targeted ESLint on touched files PASS; `npm run lint` PASS; `npm test -- --maxWorkers=1` PASS (117 files / 660 tests); preview-env `CIRCLE_NODE_TOTAL=2 npm run build` PASS with Next collecting page data using 1 worker.
- Branch/commit: `main` @ `2669c09`.
- Next:          CourtListener token is now configured in Vercel + Railway main worker, no need on `scrapling-worker`; next ops check should run `npm run verify:ingestion-runtime` in the deployed worker/runtime or inspect Railway logs for a live CourtListener probe.

2026-07-16 - Codex - T-INTERNATIONAL-STANDARDS - DONE-LOCAL
- Intent:        Add a dedicated International lane to the public Standards page so ISO/IEC AI standards, OECD AI Principles, UNESCO AI Ethics, and IEEE 7000 appear alongside Europe/US standards while preserving non-binding/incorporation caveats.
- Files:         `src/content/ai-regulation/international-ai-standards.ts`, `src/content/ai-regulation/international-ai-standards.test.ts`, `src/app/[lang]/standards/page.tsx`, `src/components/site/standards-explorer.tsx`, `README.md`, `PROJECT_LOGBOOK.md`, `AI_AGENT_MASTER_CONTEXT.md`, `AI_TASKS.md`.
- Graph anchors: `internationalAiStandardsBaseline`, `StandardsExplorer`, community "Intelligence Hub UI", community "News and Regulation Admin".
- Verification:  `npm test -- src/content/ai-regulation/international-ai-standards.test.ts` PASS (3 tests); `npm run typecheck` PASS; `npm run lint` PASS; `npm test` PASS (117 files / 660 tests); preview-env `npm run build` PASS.
- Branch/commit: `main` @ `4b170f9`.
- Next:          Claude can consume the International region filter visually without backend changes; future backend work can add monitoring sources for ISO/IEC, IEEE, OECD, and UNESCO update feeds if official machine-readable feeds become available.

2026-07-16 - Codex - T-COURTLISTENER-RUNTIME-READINESS - DONE-LOCAL
- Intent:        Make the US case-law API path operationally ready before the operator provides credentials tonight: accept `COURTLISTENER_API_KEY` or `COURTLISTENER_API_TOKEN`, expose CourtListener in runtime verification, and register the active US case-law source for Supabase.
- Files:         `src/lib/env.ts`, `src/agents/ai-regulation/connectors/api-connector.ts`, `src/agents/ai-regulation/connectors/api-connector.test.ts`, `src/agents/ai-regulation/agentApiCapabilities.ts`, `src/agents/ai-regulation/agentApiCapabilities.test.ts`, `scripts/verify-ingestion-runtime.ts`, `.env.example`, `src/db/seed/ai-regulation-seed.ts`, `src/db/migrations/024_add_us_courtlistener_case_law_source.sql`, `README.md`, `PROJECT_LOGBOOK.md`, `AI_AGENT_MASTER_CONTEXT.md`, `AI_TASKS.md`.
- Graph anchors: `env`, `ApiConnector`, `listAgentApiCapabilities()`, `regulationSourcesSeed`, community "API Connectors and Legal Docs", community "Agent API Capabilities", community "DB Repository Layer".
- Verification:  `npm test -- src/agents/ai-regulation/connectors/api-connector.test.ts src/agents/ai-regulation/agentApiCapabilities.test.ts` PASS (2 files / 19 tests); `npm run verify:ingestion-runtime` PASS with all local providers honestly `blocked_missing_credentials`, now including CourtListener; `npm run typecheck` PASS; `npm run lint` PASS; preview-env `npm run build` PASS.
- Branch/commit: `main` @ `2c274e4`.
- Next:          Operator still needs to provide/set CourtListener token tonight plus PISTE/Judilibre/EUR-Lex details as available; after `024_add_us_courtlistener_case_law_source.sql` is applied and token is set, run `npm run verify:ingestion-runtime` in the target runtime and trigger a US scan to confirm live CourtListener ingestion.

...
| T-SITE-HEALTH-AUDIT | Codex | DONE-LOCAL | `ops/t-ops9-ux` @ `9c8d658` | `src/agents/ai-regulation/processors/scanJobs.ts`, `src/lib/health.ts`, `src/lib/admin-operations-summary.ts`, `src/agents/ai-regulation/agentApiCapabilities.ts`, `scripts/verify-ingestion-runtime.ts`, related tests | `evaluateScanJobOutcome()`, `buildHealthSnapshot()`, `listAgentApiCapabilities()`, community "Scan Job Management", community "Source Runtime Health", community "Agent API Capabilities" | 2026-06-29 |

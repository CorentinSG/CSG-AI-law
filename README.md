...
EU official legal-database monitoring also includes `EUR-Lex SOAP webservice` via `apiProvider: "eurlex"` on `src-eur-lex-ai`. It requires `EURLEX_USERNAME` and `EURLEX_PASSWORD` in runtime env. EUR-Lex results are official EU legal database records, but the connector now normalizes language-prefixed titles, prefers CELEX over CELLAR references when available, extracts document dates/forms, maps instrument types into `authorityType`/`developmentType` hints, and filters out broad non-AI results before raw insertion. Broad EUR-Lex corpora must not be imported wholesale.
